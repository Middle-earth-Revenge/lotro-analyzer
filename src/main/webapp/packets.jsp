...
try {
	...
	if (request.getParameter("group") == null || request.getParameter("group").isEmpty()) {
		packets = em.createQuery("select packet from Packet packet", Packet.class).getResultList();
	} else {
		packets = em.createQuery("select packet from Packet packet where group.key = :groupKey", Packet.class).setParameter("groupKey", Integer.parseInt(request.getParameter("group"))).getResultList();
	}
...
for (Packet packet : packets) {
	if ((group == null && packet.getGroup() != null) || (group != null && !group.getKey().equals(packet.getGroup().getKey()))) {
		group = packet.getGroup();
		pageContext.setAttribute("group", group);
%>
				<tr>
					<td colspan="3" class="packetgroup"><a href="packets.jsp?group=${group.key}">${group.name}</a> <a style="font-weight: normal;" href="/export/packetgroup?group=${group.key}">Export</a></td>
				</tr>
<%
	}
...
%>
				<tr class="packetrow">
					<td><a href="packet.jsp?packet=${packet.key}">${packet.name}</a></td>
					<td>${packet.analysesSize}</td>
					<td><a href="/export/packet?packet=${packet.key}">Export</a> <a href="/export/packet?packet=${packet.key}&amp;type=binary">Export binary</a></td>
				</tr>
<%
}
%>
